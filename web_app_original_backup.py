...
for directory in ['uploads', 'codes_output', 'images_output', 'discards_output', 'rectangles_output', 'images_sources', 'images_old', 'source']:
...
def get_image_from_sources():
    """Obtiene la imagen de images_sources (debe haber solo una)"""
    images_sources = 'images_sources'
    if not os.path.exists(images_sources):
        return None
    
    files = [f for f in os.listdir(images_sources) 
             if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    
    if len(files) == 0:
        return None
    elif len(files) == 1:
        return os.path.join(images_sources, files[0])
    else:
        # Si hay más de una imagen, tomar la primera
        send_progress(f"⚠️ Hay {len(files)} imágenes en images_sources, procesando la primera")
        return os.path.join(images_sources, files[0])
...
def move_next_image_to_sources():
    ...
    target_path = os.path.join(images_sources, source_file)
    
    try:
        shutil.move(source_path, target_path)
        send_progress(f"📁 Movida imagen: {source_file} → images_sources")
        return True
...
def move_processed_image_to_old(image_path):
    """Mueve la imagen procesada de images_sources a images_old"""
...
def process_image_automated():
    ...
    try:
        # Verificar si hay imagen en images_sources
        image_path = get_image_from_sources()
        if not image_path:
            return {'error': 'No hay imagen en images_sources para procesar'}
        ...
        send_progress("🔍 Detectando rectángulos en la imagen...", 1, 4)
        extract_rectangles(
        ...
        send_progress("🤖 Clasificando rectángulos con OCR...", 2, 4)
        ...
        send_progress(f"📊 Resultado: {codes_count} códigos, {images_count} imágenes, {discards_count} descartes", 3, 4)
        
        processing_time = time.time() - start_time
        needs_manual_review = codes_count != images_count or discards_count > 0
        
        # Paso 3: Mover imagen procesada y preparar siguiente
        send_progress("📁 Organizando archivos...", 4, 4)
        
        # Mover imagen actual a images_old
        if move_processed_image_to_old(image_path):
            # Mover siguiente imagen de source a images_sources
        ...
        return {
            ...
            'message': f'Procesada {filename}: {codes_count} códigos, {images_count} imágenes, {discards_count} descartes.'
        }
...
def serve_image_source(filename):
    """Sirve imágenes desde images_sources"""
    try:
        filepath = os.path.join('images_sources', filename)
